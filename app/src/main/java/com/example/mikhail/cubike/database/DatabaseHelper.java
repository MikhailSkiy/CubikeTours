...
//        sqLiteDatabase.execSQL(SQL_CREATE_PLACES_TABLE);
    ...
    private void copyDatabase() throws IOException {
        InputStream inputFileName = context_.getAssets().open(DATABASE_NAME);

        String outFileName = getDatabasePath();
        File f = new File(context_.getApplicationInfo().dataDir + DB_PATH_SUFFIX);
        if (!f.exists())
            f.mkdir();
        ...
        int length;
        while ((length = inputFileName.read(buffer)) > 0) {
    ...
    private  String getDatabasePath() {
        ...
        return context_.getApplicationInfo().dataDir + DB_PATH_SUFFIX + DATABASE_NAME;

    }

    public SQLiteDatabase openDataBase() throws SQLiteException{
        File dbFile = context_.getDatabasePath(DATABASE_NAME);
        if (!dbFile.exists()){
            try {
                copyDatabase();
            } catch (IOException e){
